...
@implementation UIViewController (IPNavigationPopGestureRecognizer)
...
- (void)resetNavigationBar {
    if (![self.navigationController isKindOfClass:[IPNavigationController class]]) {
        return;
    }
    [(IPNavigationController *)self.navigationController app_resetNavigationBar];
}

@end

@interface IPNavigationController ()<UIGestureRecognizerDelegate>
...
- (void)addFullPopGestureRecognizerWithNavigationViewController:(UINavigationController *)nav {
    ...
    while (theVC && ![theVC isKindOfClass:[IPNavigationController class]]) {
    ...
    if ([nav isKindOfClass:[IPNavigationController class]]) {
        popGr = [(IPNavigationController *)nav fullScreenPopGestureRecognizer];
    }
    if (!popGr) {
